...
def get_llm_client():
    """config.LLM_PROVIDER에 따라 LLM 클라이언트 반환"""
...
def infer_product_group(client, provider: str, fruit_name: str) -> Optional[str]:
    """LLM으로 입력된 과일명이 어느 품목 그룹에 속하는지 판단"""

    # LLM 추론 먼저 시도 (신규 품종도 분류 가능)
...
품목그룹: [위 목록 중 하나 또는 None]"""
    ...
    try:
        if provider == "claude":
            # Claude 웹 검색 (web_search tool 사용)
            response = client.messages.create(
...
다음 형식으로 답변해주세요 (주의: 범위 표시 시 ~ 대신 - 사용):
...
def run_cold_start_prediction(fruit_name: str, store: str = None) -> None:
    ...
    fruit_name : str
        신규 상품명
    store : str, optional
        점포 코드 (None이면 전체 데이터에서 검색)
    """
    client, provider = get_llm_client()
    if not client:
        st.error(f"AI 연결 실패. {provider.upper()} API 키를 확인해주세요.")
        return

    with st.status(f"'{fruit_name}' 분석 중...", expanded=True) as status:
        st.write(f"🤖 {provider.upper()} 모델 사용")
        if store:
            st.write(f"🏬 점포: {store}")
        st.write("📋 품목 그룹을 찾고 있습니다...")

        # Step 1: 웹 검색으로 정확한 과일 정보 파악
        ...
        if group_name:
            ...
            # 웹 검색 실패 시 LLM 추론으로 폴백
            st.write("📋 LLM 추론으로 품목 그룹을 판단합니다...")
            group_name = infer_product_group(client, provider, fruit_name)

            if group_name:
                st.write(f"✅ **'{group_name}'** 그룹으로 분류되었습니다.")
            else:
                # 마지막 시도: raw_sales.csv에서 상품명 직접 검색
                st.write("🔎 데이터베이스에서 상품명을 직접 검색합니다...")
                direct_products = search_similar_products([fruit_name], store)
                if direct_products:
                    st.write(f"✅ **{len(direct_products)}개** 유사 상품을 직접 검색으로 찾았습니다.")
                    # 직접 검색 결과로 진행
                    ...
                    if top3:
                        ...
                        prediction = generate_final_prediction(client, provider, fruit_name, "직접검색", top3, stats)
                        status.update(label="분석 완료!", state="complete")

                        st.markdown("---")
                        st.markdown(f"### 📦 [{fruit_name}] 예측 결과")
                        st.markdown("**품목 그룹:** 직접 검색 (그룹 미분류)")
